package com.postnode.service.bctransaction.processors;

import com.postnode.service.bctransaction.database.entities.Transaction;
import com.postnode.service.bctransaction.database.repositories.TransactionRepository;
import com.postnode.service.bctransaction.messaging.streams.TransactionStreams;
import com.postnode.service.bctransaction.messaging.models.TransactionInfo;
...
public class DatabaseTransactionReceiptProcessor extends TransactionReceiptProcessor {
    ...
    public void update() {
        ...
        try (Locker ignored = new Locker(mutex)) {
            ...
            transactions.findAllByStatus(Transaction.TransactionStatus.PENDING).stream()
    ...
    synchronized private void updateTransactionInfo(TransactionReceipt receipt) {
        ...
        transaction.setStatus(receipt.isStatusOK() ?
                receipt.getContractAddress() != null ? Transaction.TransactionStatus.DEPLOYED : Transaction.TransactionStatus.SUCCESS :
    ...
    private void sendTransactionMessage(TransactionReceipt receipt, Transaction transaction) {
        String address = transaction.getStatus() == Transaction.TransactionStatus.DEPLOYED ? receipt.getContractAddress() : receipt.getTo();
